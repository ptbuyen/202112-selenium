...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (ClassPath.ClassInfo info : ClassPath.from(loader).getTopLevelClasses()) {
            if (info.getName().startsWith("test.") &&
                    !info.getName().equalsIgnoreCase("test.BaseTest") &&
        ...
        try {
            ...
            throw new IllegalArgumentException("[ERR] " + browser + " is not supported, we cover for " + Arrays.toString(BrowserType.values()));
        }

        // We will parallel base on maximum parallel
